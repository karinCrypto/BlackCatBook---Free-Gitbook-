...
const TYPE_META: Record<string, { label: string; emoji: string; gradient: string }> = {
  tech_docs:  { label: '기술 문서',  emoji: '📖', gradient: 'linear-gradient(135deg,#3b82f6,#6366f1)' },
  blog:       { label: '블로그',    emoji: '✍️', gradient: 'linear-gradient(135deg,#f43f5e,#fb923c)' },
  portfolio:  { label: '포트폴리오', emoji: '🎨', gradient: 'linear-gradient(135deg,#a855f7,#ec4899)' },
}
...
export default function DashboardPage() {
  ...
  function refresh() {
    ...
    ws.forEach(w => {
      counts[w.id] = getPages(w.id).filter(p => p.type === 'page').length
    })
  ...
  function handleCreate() {
    ...
    const ws = createWorkspace({
      name: name.trim(), type: type as Workspace['type'],
      theme: activeTheme, description: desc,
      emoji: wsEmoji,
    } as Parameters<typeof createWorkspace>[0])
    refresh()
    setModal(false); setName(''); setDesc(''); setWsEmoji('📖')
    router.push(`/workspace/${ws.id}`)
  }

  function handleDelete(id: string, e: React.MouseEvent) {
    e.preventDefault(); e.stopPropagation()
    if (!confirm('워크스페이스와 모든 문서가 삭제됩니다. 계속하시겠어요?')) return
    deleteWorkspace(id)
    refresh()
  }
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)' }}>
      ...
      <header style={{ height:60, background:'var(--header-bg)', borderBottom:'1px solid var(--border)',
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          <Link href="/login" style={{ fontSize:'0.82rem', color:'var(--text-muted)', padding:'6px 12px',
            borderRadius:8, textDecoration:'none' }}>로그인</Link>
      ...
      <div style={{ maxWidth:1040, margin:'0 auto', padding:'36px 24px 80px' }}>
        ...
        <div style={{ marginBottom:32, display:'flex', alignItems:'flex-end', justifyContent:'space-between', flexWrap:'wrap', gap:16 }}>
          <div>
            <h1 style={{ fontSize:'1.8rem', fontWeight:800, color:'var(--text)', marginBottom:4 }}>내 워크스페이스</h1>
            <p style={{ color:'var(--text-muted)', fontSize:'0.875rem' }}>
              로컬에 저장 — 로그인 없이 무료로 사용
            </p>
          </div>
          {/* Stats */}
          {workspaces.length > 0 && (
            <div style={{ display:'flex', gap:12 }}>
              {[
                { label:'워크스페이스', value: workspaces.length, icon:'🗂️' },
                { label:'총 페이지', value: totalPages, icon:'📄' },
              ].map(s => (
          ...
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px,1fr))', gap:16 }}>
                ...
                return (
                  <Link key={ws.id} href={`/workspace/${ws.id}`}
                    ...
                    <div style={{ height:72, background: meta.gradient, position:'relative', display:'flex', alignItems:'center', padding:'0 18px', gap:12 }}>
                      <span style={{ fontSize:28 }}>{(ws as Workspace & {emoji?:string}).emoji || meta.emoji}</span>
                      <div style={{ marginLeft:'auto', display:'flex', gap:6 }}>
                        <button onClick={e => startRename(ws, e)} title="이름 변경"
                        ...
                        <button onClick={e => handleDelete(ws.id, e)} title="삭제"
                          style={{ width:28, height:28, borderRadius:8, border:'none',
                            background:'rgba(255,255,255,.2)', color:'white', cursor:'pointer',
                            display:'flex', alignItems:'center', justifyContent:'center' }}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <div style={{ padding:'14px 18px 16px' }}>
                      {isRenaming ? (
                        <input ref={renameRef} value={renameVal}
                          ...
                          style={{ width:'100%', border:'1px solid var(--accent)', borderRadius:6,
                      ...
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop: ws.description ? 0 : 8 }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:10, fontSize:'0.75rem', color:'var(--text-faint)' }}>
                          <span>📄 {pages}페이지</span>
                          <span>{new Date(ws.createdAt).toLocaleDateString('ko-KR', { month:'short', day:'numeric' })}</span>
                        </div>
                      </div>
                    </div>
                  </Link>
              ...
              <button onClick={() => setModal(true)}
                ...
                <div style={{ width:40, height:40, borderRadius:'50%', border:'2px dashed currentColor',
                  display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>+</div>
                <span style={{ fontSize:'0.85rem', fontWeight:600 }}>새 워크스페이스</span>
              </button>
            </div>
          </>
        )}
      ...
      <div style={{ position:'fixed', bottom:24, right:24, zIndex:100 }}>
        {themePanel && (
          <div style={{ marginBottom:8, padding:12, borderRadius:14, background:'var(--bg)',
            border:'1px solid var(--border)', boxShadow:'var(--shadow-lg)', minWidth:160 }}>
      ...
      {modal && (
        <div style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,.55)',
          ...
          <div style={{ background:'var(--bg)', border:'1px solid var(--border)', borderRadius:18,
            ...
            <div style={{ padding:'20px 24px 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div style={{ position:'relative' }}>
                <button onClick={() => setEmojiPicker(o=>!o)}
                  style={{ width:44, height:44, borderRadius:12, border:'1px solid var(--border)',
                    background:'var(--bg-secondary)', fontSize:22, cursor:'pointer' }}>
                  {wsEmoji}
                </button>
                {emojiPicker && (
                  <>
                    <div style={{ position:'fixed', inset:0, zIndex:299 }} onClick={() => setEmojiPicker(false)} />
                    <div style={{ position:'absolute', top:'110%', left:0, zIndex:300,
                      background:'var(--bg)', border:'1px solid var(--border)', borderRadius:12,
                      padding:10, boxShadow:'var(--shadow-lg)', display:'flex', flexWrap:'wrap', gap:4, width:200 }}>
                      {EMOJIS.map(em => (
                        <button key={em} onClick={() => { setWsEmoji(em); setEmojiPicker(false) }}
                          style={{ width:34, height:34, borderRadius:8, border:'none', background:'transparent',
              ...
              <div style={{ flex:1 }}>
                <h3 style={{ color:'var(--text)', fontWeight:800, fontSize:'1.05rem', margin:0 }}>새 워크스페이스</h3>
                <p style={{ color:'var(--text-faint)', fontSize:'0.75rem', marginTop:2 }}>이모지를 눌러 아이콘을 바꿀 수 있어요</p>
              </div>
              <button onClick={() => setModal(false)}
                style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-faint)', fontSize:20 }}>×</button>
            </div>

            <div style={{ padding:'20px 24px 24px' }}>
              <label style={{ fontSize:'0.78rem', fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.05em' }}>이름 *</label>
              <input style={inputStyle} value={name} onChange={e => setName(e.target.value)}
                placeholder="내 프로젝트 문서" autoFocus
                onKeyDown={e => e.key==='Enter' && handleCreate()} />

              <label style={{ fontSize:'0.78rem', fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.05em', display:'block', marginTop:16 }}>유형</label>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:6 }}>
                {TYPES.map(t => (
                  <button key={t.id} onClick={() => { setType(t.id); setWsEmoji(TYPE_META[t.id].emoji) }}
                    style={{ padding:'10px 8px', borderRadius:10, textAlign:'center',
                      ...
                      cursor:'pointer' }}>
                    <div style={{ fontSize:20, marginBottom:4 }}>{TYPE_META[t.id].emoji}</div>
                    <div style={{ fontSize:'0.75rem', fontWeight:700,
                      color: type===t.id ? 'var(--accent-text)' : 'var(--text)' }}>{TYPE_META[t.id].label}</div>
                  </button>
                ))}
              </div>

              <label style={{ fontSize:'0.78rem', fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.05em', display:'block', marginTop:16 }}>설명 <span style={{ fontWeight:400, textTransform:'none' }}>(선택)</span></label>
              <input style={inputStyle} value={desc} onChange={e => setDesc(e.target.value)} placeholder="간단한 설명을 입력하세요" />
              ...
              <div style={{ display:'flex', gap:10, marginTop:20 }}>
                ...
                <button onClick={handleCreate} disabled={!name.trim()}
                  style={{ flex:2, padding:'11px', borderRadius:10, border:'none',
                    background:'var(--accent)', color:'white', cursor:'pointer', fontWeight:700,
                    opacity: name.trim() ? 1 : 0.45, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                  워크스페이스 만들기
                </button>
              </div>
