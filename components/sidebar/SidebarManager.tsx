...
export default function SidebarManager({
  ...
  const filtered = search.trim()
    ? pages.filter(p => p.title.toLowerCase().includes(search.toLowerCase()) ||
        p.content.replace(/<[^>]*>/g,'').toLowerCase().includes(search.toLowerCase()))
    : []

  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      {/* Workspace name + Add buttons */}
      <div style={{ padding:'14px 12px 10px', borderBottom:'1px solid var(--border)' }}>
        ...
        {/* Add buttons — RIGHT HERE, not at bottom */}
        <div style={{ display:'flex', gap:6 }}>
          <button onClick={() => handleCreate(null, 'page')}
            style={{ flex:1, padding:'6px', borderRadius:7, border:'1px solid var(--border)',
      ...
      <div style={{ padding:'8px 12px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 10px',
          borderRadius:8, background:'var(--bg-tertiary)', border:'1px solid var(--border)' }}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)}
            placeholder="검색..." style={{ flex:1, border:'none', background:'transparent',
      ...
      {search.trim() && (
        <div style={{ flex:1, overflow:'auto', padding:'0 8px' }}>
          {filtered.length === 0 ? (
            <div style={{ textAlign:'center', padding:'20px 0', fontSize:'0.82rem', color:'var(--text-faint)' }}>결과 없음</div>
          ) : filtered.map(p => (
            <button key={p.id} onClick={()=>{ onNavigate(p.id); setSearch('') }}
              style={{ display:'block', width:'100%', textAlign:'left', padding:'8px 10px', borderRadius:8,
                border:'none', background: currentPageId===p.id ? 'var(--accent-light)' : 'transparent',
                color: currentPageId===p.id ? 'var(--accent-text)' : 'var(--text)', cursor:'pointer',
      ...
      {!search.trim() && (
        <>
          <div style={{ flex:1, overflow:'auto', padding:'4px 8px' }}>
            {tree.length === 0 ? (
              <div style={{ textAlign:'center', padding:'32px 0', fontSize:'0.82rem', color:'var(--text-faint)' }}>
                아직 페이지가 없어요
              </div>
            ) : tree.map(node => (
