...
export const NOTE_TEMPLATES: NoteTemplate[] = [
  ...
  },
  {
    ...
    emoji: '🌈',
    label: '버킷리스트',
    desc: '카테고리별 하고 싶은 일 100',
    title: '🌈 나의 버킷리스트',
    html: [
      '<p>이루면 ✅, 진행 중이면 🔥 를 붙여보세요!</p>',
      ...['✈️ 여행', '💪 도전', '📚 배움', '💰 재정', '❤️ 관계', '🎨 취미'].map(cat =>
        `<h2>${cat}</h2><ul><li>☐ &nbsp;</li><li>☐ &nbsp;</li><li>☐ &nbsp;</li></ul>`),
      '<h2>🏆 올해 안에 꼭 이룰 TOP 3</h2>',
      '<ol><li>&nbsp;</li><li>&nbsp;</li><li>&nbsp;</li></ol>',
    ].join('\n'),
  },
  {
    ...
    html: [
      '<h2>올해의 한 문장</h2><p style="font-size:1.2em"><strong>"&nbsp;"</strong></p>',
      '<h2>영역별 목표</h2>',
      '<table><tr><th>영역</th><th>목표</th><th>측정 방법</th><th>달성</th></tr>',
      ['커리어/학업', '건강', '재정', '관계', '성장/취미'].map(a => `<tr><td><strong>${a}</strong></td><td>&nbsp;</td><td>&nbsp;</td><td>___%</td></tr>`).join(''),
      '</table>',
      '<h2>분기별 로드맵</h2>',
      '<table><tr><th>분기</th><th>핵심 과제</th><th>마일스톤</th></tr>',
      ['1분기 (1~3월)', '2분기 (4~6월)', '3분기 (7~9월)', '4분기 (10~12월)'].map(q => `<tr><td><strong>${q}</strong></td><td>&nbsp;</td><td>&nbsp;</td></tr>`).join(''),
      '</table>',
      '<h2>연말의 나에게</h2><p>&nbsp;</p>',
    ].join('\n'),
  },
  {
    ...
    label: '월 달력',
    desc: '한 달 계획을 달력 형태로',
    title: `📆 ${new Date().getMonth() + 1}월 달력`,
    html: [
      '<h2>이번 달 핵심 목표</h2><ul><li>&nbsp;</li><li>&nbsp;</li></ul>',
      `<table><tr>${['일', '월', '화', '수', '목', '금', '토'].map(d => `<th>${d}</th>`).join('')}</tr>`,
      Array.from({ length: 5 }, () => `<tr>${Array.from({ length: 7 }, () => '<td style="height:70px;vertical-align:top">&nbsp;</td>').join('')}</tr>`).join(''),
      '</table>',
      '<h2>이번 달 기념일 · 마감</h2>',
      '<table><tr><th>날짜</th><th>내용</th></tr><tr><td>&nbsp;</td><td>&nbsp;</td></tr><tr><td>&nbsp;</td><td>&nbsp;</td></tr></table>',
    ].join('\n'),
  },
  {
    id: 'sheet',
    emoji: '🧮',
    label: '엑셀형 표',
    desc: '10×8 빈 스프레드시트 표',
    title: '🧮 데이터 표',
    html: [
      '<p>엑셀처럼 자유롭게 채워 쓰는 표예요. 행이 더 필요하면 마지막 행을 복사하세요.</p>',
      `<table><tr><th>#</th>${['A', 'B', 'C', 'D', 'E', 'F', 'G'].map(c => `<th>${c}</th>`).join('')}</tr>`,
      Array.from({ length: 10 }, (_, i) => `<tr><td><strong>${i + 1}</strong></td>${Array.from({ length: 7 }, () => '<td>&nbsp;</td>').join('')}</tr>`).join(''),
      '</table>',
      '<h2>합계 / 메모</h2><p>&nbsp;</p>',
    ].join('\n'),
  },
